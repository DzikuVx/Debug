 ...
 * @link http://www.shtr.eu
 * @version 0.6.5
 * @copyright 2009 Lynx-IT Paweł Stanislaw Spychalski
 * @license MIT
...
class Debug
{
     ...
     */
    static public function errorHandler($errorNumber, $errorString, $errorFile, $errorLine)
    {

        /*
         * Is line has been suppressed with an @, do not throw an exception
         */
        if (error_reporting() == 0) {
            return;
        }

        $errorFullText = self::formatErrorText($errorNumber, $errorString, $errorFile, $errorLine);

        /*
     * Hold execution
     */
        if (self::$errorHoldsExecution) {
            throw new DebugException ($errorFullText, $errorNumber);
        } else {
            /*
     * display error
     */
